...
suite(
  ...
  func() {
    test(
      "purgeOldDatapoints removes datapoints older than retention period",
      func() {
        let registry = MetricModel.emptyRegistry();
        let datapoints = MetricModel.emptyDatapoints();
        ...
        switch (result) {
          case (#ok(id)) {
            let nanosPerDay = MetricModel.NANOS_PER_DAY;

            // Add datapoints at different days
            ignore MetricModel.recordDatapoint(datapoints, registry, id, 10.0, #manual("test"), 0); // Day 0
            ignore MetricModel.recordDatapoint(datapoints, registry, id, 20.0, #manual("test"), 10 * nanosPerDay); // Day 10
            ignore MetricModel.recordDatapoint(datapoints, registry, id, 30.0, #manual("test"), 25 * nanosPerDay); // Day 25
            ignore MetricModel.recordDatapoint(datapoints, registry, id, 40.0, #manual("test"), 35 * nanosPerDay); // Day 35

            // All datapoints present before purge
            let before = MetricModel.getDatapoints(datapoints, id, null);
            expect.nat(before.size()).equal(4);

            // Purge at day 40 (retention is 30 days, so cutoff is day 10)
            let now = 40 * nanosPerDay;
            ignore MetricModel.purgeOldDatapoints(datapoints, registry, now);

            // Only datapoints from day 10+ should remain
            let after = MetricModel.getDatapoints(datapoints, id, null);
            expect.nat(after.size()).equal(3); // Day 10, 25, 35
          };
          case (#err(_)) {
            expect.bool(false).equal(true);
          };
    ...
    test(
      "purgeOldDatapoints removes all datapoints for deleted metrics",
      func() {
        let registry = MetricModel.emptyRegistry();
        let datapoints = MetricModel.emptyDatapoints();

        // Manually add datapoints for a non-existent metric
        Map.add(
          datapoints,
          Nat.compare,
          999,
          [
            { timestamp = 1000; value = 10.0; source = #manual("test") },
          ],
        );

        let before = MetricModel.getDatapoints(datapoints, 999, null);
        expect.nat(before.size()).equal(1);

        // Purge should remove datapoints for metrics not in registry
        ignore MetricModel.purgeOldDatapoints(datapoints, registry, testTimestamp);

        let after = MetricModel.getDatapoints(datapoints, 999, null);
        expect.nat(after.size()).equal(0);
      },
    );
  },
);
