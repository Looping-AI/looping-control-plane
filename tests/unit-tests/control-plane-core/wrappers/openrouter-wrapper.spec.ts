...
describe("OpenRouter Wrapper Unit Tests", () => {
  ...
  describe("Chat Method Tests", () => {
    it("should fail with invalid model name", async () => {
      for (const model of ["", "   "]) {
        try {
  ...
  describe("Built-In Tools (useBuiltInTool) Tests", () => {
    ...
    it("should handle visit website with single URL", async () => {
      ...
      if ("ok" in response) {
        ...
        expect(
          lowerContent.includes("openrouter") || lowerContent.includes("model"),
        ).toBe(true);
    ...
    it("should fail with invalid API key", async () => {
      for (const apiKey of ["", "   "]) {
        try {
          await testCanister.openRouterUseBuiltInTool(apiKey, "Test message", {
            web_search: { searchSettings: [] },
          });
          expect(false).toBe(true); // Should not reach here
        } catch (error) {
