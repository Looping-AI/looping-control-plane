# mo:core InternetComputer module

Detailed reference for `mo:core/InternetComputer`, preserving the exact wording, cautions, and examples from the upstream docs.
...
- Low-level interface to the Internet Computer system API.
- Provides raw call primitives plus performance counters, instruction counting, and reply deadlines.
...
This file mirrors `src/InternetComputer.mo` so AI tooling has immediate access to the canonical examples.
