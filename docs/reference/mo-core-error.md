# mo:core Error module

Detailed reference for `mo:core/Error`, preserving the exact wording, cautions, and examples from the upstream docs.
...
- Defines the opaque `Error` type used with `throw`/`catch`.
- Exposes `ErrorCode` variants describing system vs canister issues, plus helper functions for inspection.
...
This file mirrors `src/Error.mo` so AI tooling has immediate access to the canonical examples.
