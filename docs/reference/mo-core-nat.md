...
Authoritative reference for `mo:core/Nat`, preserving the upstream documentation text and examples.
...
- `Nat` is arbitrary-precision unsigned, so addition/multiplication never overflow.
- Subtraction/division/rem trap on negative results.
- Helpers mirror built-in operators so you can pass them to higher-order functions.
...
This file mirrors `src/Nat.mo` so AI tooling has immediate access to the canonical examples.
