...
High-level reference for `mo:core/List`, focusing on the most useful operations and including the official examples from the core library.
...
- `List<T>` is a **mutable, growable array** with O(1) random access and O(sqrt(n)) memory overhead.
- Unlike `pure/List` (a functional linked list), this `List` is imperative: elements can be added, removed, and overwritten in place.
- Can be declared `stable` for orthogonal persistence across upgrades.
- The maximum number of elements is 2^32.
- Based on "Resizable Arrays in Optimal Time and Space" (Brodnik et al., 1999).
...
///
/// Runtime: `O(size)`
public func fromIter<T>(iter : Iter<T>) : List<T>;
...
public func addAll<T>(self : List<T>, iter : Iter<T>);
...
public func toText<T>(self : List<T>, toText : T -> Text) : Text;
...
///
/// *Runtime and space assumes that `f` runs in O(1) time and space.
public func mapResult<T, R, E>(self : List<T>, f : T -> Result<R, E>) : Result<List<R>, E>;
...
public func flatMap<T, R>(self : List<T>, k : T -> Iter<R>) : List<R>;
...
///
/// Space: O(number of elements in list)
public func join<T>(self : Iter<List<T>>) : List<T>;
...
public func sort<T>(self : List<T>, compare : (T, T) -> Order) : List<T>;
...
/// Space: O(size)
/// *Runtime and space assumes that `compare` runs in O(1) time and space.
public func sortInPlace<T>(self : List<T>, compare : (T, T) -> Order);
...
///
/// Space: O(1)
public func deduplicate<T>(self : List<T>, equal : (T, T) -> Bool);
...
public func contains<T>(self : List<T>, equal : (T, T) -> Bool, element : T) : Bool;
...
///
/// Space: O(1)
public func isSorted<T>(self : List<T>, compare : (T, T) -> Order) : Bool;
...
///
/// *Runtime and space assumes that `equal` runs in `O(1)` time and space.
public func indexOf<T>(self : List<T>, equal : (T, T) -> Bool, element : T) : ?Nat;
...
///
/// *Runtime and space assumes that `equal` runs in O(1) time and space.
public func nextIndexOf<T>(self : List<T>, equal : (T, T) -> Bool, element : T, fromInclusive : Nat) : ?Nat;
...
///
/// *Runtime and space assumes that `equal` runs in `O(1)` time and space.
public func lastIndexOf<T>(self : List<T>, equal : (T, T) -> Bool, element : T) : ?Nat;
...
///
/// Space: O(1)
public func prevIndexOf<T>(self : List<T>, equal : (T, T) -> Bool, element : T, fromExclusive : Nat) : ?Nat;
...
///
/// *Runtime and space assumes that `compare` runs in `O(1)` time and space.
public func binarySearch<T>(self : List<T>, compare : (T, T) -> Order, element : T) : {
  #found : Nat;
  #insertionIndex : Nat;
};
...
///
/// *Runtime and space assumes that `compare` runs in O(1) time and space.
public func max<T>(self : List<T>, compare : (T, T) -> Order) : ?T;
...
///
/// *Runtime and space assumes that `compare` runs in O(1) time and space.
public func min<T>(self : List<T>, compare : (T, T) -> Order) : ?T;
...
///
/// Runtime: `O(1)`
public func values<T>(self : List<T>) : Iter<T>;
...
///
/// Runtime: `O(1)`
public func reverseValues<T>(self : List<T>) : Iter<T>;
...
///
/// Runtime: `O(1)`
public func keys<T>(self : List<T>) : Iter<Nat>;
...
public func enumerate<T>(self : List<T>) : Iter<(Nat, T)>;
...
public func reverseEnumerate<T>(self : List<T>) : Iter<(Nat, T)>;
...
///
/// Space: O(1)
public func range<T>(self : List<T>, fromInclusive : Int, toExclusive : Int) : Iter<T>;
...
### `reverseForEach` / `reverseForEachEntry`
...
///
/// *Runtime and space assumes that `equal` runs in O(1) time and space.
public func equal<T>(self : List<T>, other : List<T>, equal : (T, T) -> Bool) : Bool;
...
///
/// *Runtime and space assumes that `compare` runs in O(1) time and space.
public func compare<T>(self : List<T>, other : List<T>, compare : (T, T) -> Order) : Order;
