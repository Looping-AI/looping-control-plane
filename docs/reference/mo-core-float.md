# mo:core Float module

Detailed reference for `mo:core/Float`, preserving the exact wording, cautions, and examples from the upstream docs.
...
- Double precision (64-bit) IEEE 754 numbers; precision is limited and arithmetic can introduce rounding errors.
- Floating point comparisons with `==`/`!=` are discouraged; prefer epsilon-based helpers (`Float.equal`).
- Documentation uses the special values `+inf`, `-inf`, and `NaN` exactly as in the core library.
...
This file mirrors `src/Float.mo` so AI tooling has immediate access to the canonical examples.
