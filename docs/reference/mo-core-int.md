...
Mirror of the `mo:core/Int` documentation so downstream tools inherit the exact wording and examples.
...
- `Int` values are arbitrary-precision signed integers; no overflow in core arithmetic.
- Many helpers duplicate built-in operators so they can be passed as function values.
...
## Conversions with Nat and Float
...
This file mirrors `src/Int.mo` so AI tooling has immediate access to the canonical examples.
