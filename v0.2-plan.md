...
- Add private functions matching Slack API methods: `users_list()`, `conversations_list()`, `conversations_members()`. Parameters aligned with Slack's API.
...
- New model: `Map<SlackUserId, UserCacheEntry>`.
- `UserCacheEntry = { slackUserId, displayName, isPrimaryOwner, isOrgAdmin, workspaceMemberships: [(workspaceId, #admin | #member)] }`.
...
## Phase 1 — Slack-Only Write Surface
...
### 1.1 — Audit and remove external update methods
...
### 1.3 — App install and setup flow
...
### 1.4 — Channel ID anchoring logic

- On weekly reconciliation: verify all channel IDs.
- Recovery mode for org admin channel: DM to Primary Owner.
- Recovery mode for workspace admin channel: notify `#looping-ai-org-admins`.
- Recovery mode for workspace member channel: notify workspace admin channel.

---

## Phase 2 — Agent Registry & Routing

**Goal**: Introduce the agent registry, `::` syntax, session tracking, and the Agent Router with round control.

### 2.1 — Agent registry model
...
### 2.4 — UserAuthContext round tracking
...
### 2.6 — Refactor current service into generic agent service
